...
class JobService {
  ...
  async createJob({
    ...
    const service =
      await serviceService.validateServiceForJob(
        serviceId
      );
    ...
    if (
      ...
      if (!requestedWorker) {
        throw new Error(
          "Assigned worker not found or invalid"
        );
      }
    }
    ...
    await Promise.all(
      ...
      admins.map((admin) =>
        ...
        createNotification({
          ...
          message:
            `${job.title} was created by a client (${bookingType} booking)`,
    ...
    // ================= DIRECT TECHNICIAN NOTIFICATION =================
  ...
  async acceptJob(jobId, workerId) {
    ...
    await serviceService.incrementBookingCount(
      job.serviceId
    );
    ...
    await createNotification({
      ...
      message:
        "A technician accepted your job",
  ...
  async rejectJob(
    ...
    if (
      ...
      throw new Error(
        "You can only reject jobs assigned to you"
      );
    }
